...
- IVT (شار رطوبتی خالص)
- دورپیوندها (ENSO, NAO, AMOC, EA, WP, PNA, ...)
...
# مسیر داده‌های IVT - به‌صورت خودکار پیدا می‌شود
# (از فایل‌های ماهانه یا سالانه)
...
if len(ivt_files) > 0:
    # اولویت با monthly_South.csv یا monthly_Center.csv یا monthly_North.csv
    preferred = [f for f in ivt_files if "monthly_South" in f or "monthly_Center" in f]
    if preferred:
        IVT_FILE = preferred[0]
    else:
        IVT_FILE = ivt_files[0]
...
if IVT_FILE and os.path.exists(IVT_FILE):
    ...
    print(f"✅ IVT بارگذاری شد: {len(df_ivt_raw)} رکورد")
    
    # اگر فایل ماهانه است، سالانه کنیم
    if 'month' in df_ivt_raw.columns:
        # شناسایی ستون‌های شار (inflow/outflow)
        ...
        if len(ivt_cols) > 0:
            ...
            # اگر ستون‌های استاندارد نبود، ستون‌های عددی را جمع کن
        ...
        # فرض می‌کنیم قبلاً سالانه است
        if 'net_ivt' in df_ivt_raw.columns:
            df_ivt = df_ivt_raw[['year', 'net_ivt']].copy()
        else:
            # تلاش برای محاسبه net_ivt از ستون‌های موجود
            ...
            if len(ivt_cols) > 0:
                ...
                # fallback: استفاده از ستون‌های عددی
                numeric_cols = df_ivt_raw.select_dtypes(include=[np.number]).columns
    ...
    # داده‌های IVT ساختگی
    print("⚠️ استفاده از داده‌های IVT ساختگی (برای تست)")
...
if os.path.exists(INDICES_FILE):
    ...
    # محاسبه میانگین سالانه برای هر شاخص
    df_tele_annual = df_indices.groupby('year').mean(numeric_only=True).reset_index()
    
    # انتخاب ستون‌های مورد نظر (همه شاخص‌های موجود)
    tele_cols = ['NAO', 'EA', 'WP', 'EP/NP', 'PNA', 'EA/WR', 'SCA', 'POL', 'SOI',
                 'Niño 1+2', 'Niño 1+2.1', 'Niño 3', 'Niño 3.1', 'Niño 4',
                 'Niño 4.1', 'Niño 3.4', 'Niño 3.4.1', 'BV_ENSO', 'ENSO_Prec',
                 'MV_ENSO', 'ONI']
    
    # فقط ستون‌هایی که در دیتافریم وجود دارند را نگه دار
    available_cols = [col for col in tele_cols if col in df_tele_annual.columns]
    
    # همچنین ستون‌های دیگری که ممکن است وجود داشته باشند
    extra_cols = [col for col in df_tele_annual.columns if col not in ['year', 'month', 'date'] and col not in tele_cols]
    all_cols = ['year'] + available_cols + extra_cols
    
    df_tele = df_tele_annual[all_cols].copy()
    
    # تغییر نام ستون‌ها برای سادگی (اختیاری)
    rename_map = {
        'Niño 3.4': 'nino34',
        'Niño 3.4.1': 'nino34_1',
        'Niño 1+2': 'nino12',
    ...
    print(f"   شاخص‌های موجود: {', '.join([c for c in df_tele.columns if c != 'year'])}")
else:
    ...
    years = np.arange(1992, 2026)
    np.random.seed(44)
    amoc = 15 + 0.02 * (years - 1992) + np.random.normal(0, 0.5, len(years))
    df_amoc = pd.DataFrame({'year': years, 'amoc': amoc})

    df_tele = df_nino.merge(df_nao, on='year', how='outer')
    df_tele = df_tele.merge(df_amoc, on='year', how='outer')
...
# انتخاب ویژگی‌ها
feature_cols = ['net_ivt'] + [c for c in df_train.columns if c not in ['year', 'sea_level', 'net_ivt'] and df_train[c].notna().all()]
print(f"\n📊 ویژگی‌های انتخاب‌شده: {len(feature_cols)} متغیر")
...
# برای پیش‌بینی آینده، از میانگین ۲۰۱۶-۲۰۲۵ استفاده می‌کنیم
...
last_obs_var = None
try:
    # داده‌های VAR باید ایستا باشند
    data_var = df_train[['sea_level'] + feature_cols].diff().dropna()
    if len(data_var) > 10:
        model_var = VAR(data_var)
        fitted_var = model_var.fit(maxlags=2)
        models['VAR'] = fitted_var
        # ذخیره آخرین k_ar مشاهده برای پیش‌بینی
...
for name, model in models.items():
    ...
    if name == 'ARIMA':
        try:
            ...
            print(f"⚠️ خطا در پیش‌بینی ARIMA: {e}")
        ...
        try:
            if last_obs_var is not None:
                ...
                pred_vals = forecast[:, 0]  # فقط ستون sea_level
                predictions[name] = pred_vals
            else:
                predictions[name] = np.full(len(future_years), np.nan)
        except Exception as e:
            print(f"⚠️ خطا در پیش‌بینی VAR: {e}")
            predictions[name] = np.full(len(future_years), np.nan)
        ...
        try:
            ...
            print(f"⚠️ خطا در پیش‌بینی {name}: {e}")
            predictions[name] = np.full(len(future_years), np.nan)
...
# نمایش خلاصه پیش‌بینی‌ها
print("\n📊 خلاصه پیش‌بینی‌ها:")
for name, pred in predictions.items():
    print(f"  {name}: {pred}")

# ============================================================
# ۷. ارزیابی با Cross-Validation
# ============================================================
...
# ۸.۱ جدول پیش‌بینی‌ها
df_pred = pd.DataFrame({'year': future_years})
for name, pred in predictions.items():
    df_pred[name] = pred

# اضافه کردن فاصله اطمینان برای رگرسیون چندگانه
...
df_pred.to_csv(os.path.join(OUTPUT_DIR, 'forecast_comparison.csv'), index=False)

# ۸.۲ نمودار مقایسه پیش‌بینی‌ها
fig, ax = plt.subplots(figsize=(14, 8))
colors = ['blue', 'green', 'red', 'purple', 'orange', 'brown', 'pink', 'cyan', 'magenta', 'olive']
...
ax.set_title('Comparison of Forecast Methods (2026-2030)')
ax.legend()
...
# ۸.۳ اهمیت متغیرها (Random Forest)
...
with open(os.path.join(OUTPUT_DIR, 'summary_report.txt'), 'w', encoding='utf-8') as f:
    f.write("="*80 + "\n")
    f.write("🌊 گزارش پیش‌بینی جامع تراز آب دریای خزر\n")
    f.write("="*80 + "\n\n")
    f.write(f"دوره آموزش: {TRAIN_START}-{TRAIN_END}\n")
    f.write(f"دوره پیش‌بینی: {TEST_START}-{TEST_END}\n")
    f.write(f"تعداد ویژگی‌ها: {len(feature_cols)}\n\n")
